...
srresnet_checkpoint = "./saved_models/srresnetbs16/checkpoint_srresnet_129.pth.tar"  # filepath of the trained SRResNet checkpoint used for initialization
...
# Learning parameters
checkpoint = "saved_models\srganbs16\checkpoint_srgan.pth_17.tar"  # path to model (SRGAN) checkpoint, None if none
batch_size = 16  # batch size
start_epoch = 0  # start at this epoch
iterations = 2e5  # number of training iterations # 2e5
...
def main():
    ...
    for epoch in range(start_epoch, epochs):
        ...
        torch.save({'epoch': epoch,
                    ...
                    'optimizer_g': optimizer_g,
                    'optimizer_d': optimizer_d},
                   './saved_models/srganbs16/checkpoint_srgan.pth_%d.tar'%epoch)
...
def train(train_loader, generator, discriminator, truncated_vgg19, content_loss_criterion, adversarial_loss_criterion,
    ...
    for i, (lr_imgs, hr_imgs) in enumerate(train_loader):
        ...
        # Print status
        if i % print_freq == 0:
            custom_print(message,"./logs/srganbs16.txt")
